...
security <-  CNTRY_SEC_CLRNCE_T %>%
  ...
  select(CNTRY_SEC_CLRNCE_T, SEC_CLRNCE_TYP_T)
...
#quick testing of country results
skip <- F
if(skip) {
  facts.input <- facts.input %>%
    mutate(type = ifelse(runif(0,1, n = n()) <= .2, 'TEST', 'TRAIN'))
...
## Clark csv output
setwd("~/Demand Forecasting III/Templates/csv_refresh")

#Remove all old files from Output Reports
unlink("C:/Users/SCIP2/Documents/Demand Forecasting III/Output Reports/*")
...
for(i in 1:length(cartographer)) {
  ...
  openxlsx::saveWorkbook(wb = wb, 
                        file = paste0('30_day_', cartographer[i],'_', Sys.Date(), '.xlsx'))
